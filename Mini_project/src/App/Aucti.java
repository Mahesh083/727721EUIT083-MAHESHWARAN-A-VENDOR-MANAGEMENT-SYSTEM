...
import AuctionItems.Items;
import Auctions.Auction;
import Auctions.UserInterface; 
import JDBC.Jdbc;

public abstract class Aucti implements UserInterface {
    private static Connection connection;
    private static List<Auction> users = new ArrayList<>();
        ...
        try {
            ...
            if (affectedRows == 1) {
                ...
                if (generatedKeys.next()) {
                    ...
                    Auction newUser = new Auction(userId, username, email);
                    users.add(newUser);
    ...
    private static void placeBid(Scanner scanner) {
        ...
        AuctionItem item = findAuctionItem(itemId);
        Auction bidder = findUserByUsername(username);
    ...
    private static AuctionItem findAuctionItem(int itemId) {
        for (AuctionItem item : auctionItems) {
            if (item.getId() == itemId) {
                System.out.println("Found item with ID: " + itemId); // Debug statement
                return item;
            }
        }
        System.out.println("Item not found with ID: " + itemId); // Debug statement
        return null;
    }

    private static Auction findUserByUsername(String username) {
        String lowercaseUsername = username.toLowerCase();
        for (Auction user : users) {
            if (user.getUsername().equalsIgnoreCase(lowercaseUsername)) {
                return user;
            }
        }
        return null;
